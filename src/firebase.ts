...
if (firebaseConfig.apiKey === "REPLACED_BY_ENV_VAR" || !firebaseConfig.apiKey) {
  console.error("CRITICAL: Firebase API Key is missing or using placeholder 'REPLACED_BY_ENV_VAR'.");
  console.error("Please add 'VITE_FIREBASE_API_KEY' to the Secrets panel in AI Studio.");
} else {
  console.log("Firebase API Key is present (starts with:", firebaseConfig.apiKey.substring(0, 6) + "...)");
}

if (!firebaseConfig.authDomain || firebaseConfig.authDomain.includes("TODO")) {
  console.warn("Firebase Auth Domain might be incorrect or missing.");
}
